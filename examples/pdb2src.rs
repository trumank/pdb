...
impl Config {
    ...
    pub fn with_skip_methods(mut self, skip_methods: bool) -> Self {
...
fn main() {
    ...
    opts.optflag(
        "",
        "no-methods",
        "skip member functions and virtual functions",
    );
    ...
    let config = Config::new(language)
        .with_skip_dependencies(matches.opt_present("no-deps"))
        .with_skip_methods(matches.opt_present("no-methods"));
